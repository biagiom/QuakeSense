...
#define SLEEP_TIME_MIN 1  // set sleep time to 1 min 
...
void sendLoraPacket(String data) {
  ...
  // build message ID according to LoRa settings:
  if (LORA_BW == 125E3) {
    msgID = 0;
  }
  else if (LORA_BW == 250E3) {
    msgID = 100;
  }
  else if (LORA_BW == 500E3) {
    msgID = 200;
  }
  msgID += LORA_SF;
  if (LORA_CR == 8) {
  ...
  LoRa.write(msgID);           // add packet ID
...
bool readLoRaPacket(int packetSize) {
  ...
  byte payloadLen = LoRa.read();    // payload length
