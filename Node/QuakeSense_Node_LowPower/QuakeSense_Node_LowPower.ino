/*
    ...
    Node in deep sleep low-power mode (STM32 standby mode) in order
    ...
    1) At least one sensor node consisting of:
       ...
       > Dragino LoRa/GPS Shield including
         ...
         - a Quectel L80 GPS module based on MTK MT3339

    2) A LoRa gateway used to receive environmental data and messages
       from the sensor nodes and to send them to an IoT platform.
  ...
  if (n > 0) {
    ...
    if (loraInit) {
      ...
      if (isGPSDataValid) {
        ...
        if (GPS.minute < 10)
          msg += "0";
        msg += String(GPS.minute) +  ":";
        msg += String(GPS.seconds);
      }
