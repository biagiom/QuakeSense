 ...
 *  NOTE: This is an experimental implementation that adds support of ack
 *  that is sent by the gateway to the node to notify it about the sending
 *  of the data to the cloud platform.
 *  
...
byte localAddress = 0xBB;         // address of this device (LoRa Gateway)
byte destinationAddress = 0xAA;   // destination to send to (LoRa Node)
...
void setup() {
  ...
  while (WiFi.status() != WL_CONNECTED) {
    Serial.print("Attempting to connect to the network named: ");
...
void loop() {
  ...
  if(sendACK == true) {
    ...
    sendLoraACKPacket();
    SerialPort.println("ACK sent to LoRa Node 0x" + String(destinationAddress, HEX));
    
    restartLoRaModule(); // restart LoRa module and go back into receive mode
